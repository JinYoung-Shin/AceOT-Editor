...
EditorSocketIOServer.prototype.addClient = function (socket) {
  ...
  socket
    ...
    .on('operation' + this.docId, function (revision, operation, selection) {
    ...
    .on('selection' + this.docId, function (obj) {
      self.mayWrite(socket, function (mayWrite) {
        if (!mayWrite) {
          console.log("User doesn't have the right to edit.");
          return;
...
EditorSocketIOServer.prototype.updateSelection = function (socket, selection) {
  ...
  socket.broadcast['in'](this.docId).emit('selection' + this.docId, clientId, selection);
};

EditorSocketIOServer.prototype.setName = function (socket, name) {
  var clientId = socket.id;
  this.getClient(clientId).name = name;
  socket.broadcast['in'](this.docId).emit('set_name' + this.docId, clientId, name);
};
...
EditorSocketIOServer.prototype.onDisconnect = function (socket) {
  ...
  socket.broadcast['in'](this.docId).emit('client_left' + this.docId, clientId);
};
